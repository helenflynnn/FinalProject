...
```{r}
raw1980 <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/1980")
raw1999 <- httr::GET(url ="https://irmaservices.nps.gov/v3/rest/stats/total/1999")
raw2018 <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/2018")


unpacked_1980 <- httr::content(raw1980, as = "text", encoding = "UTF-8") 
unpacked_1999 <- httr::content(raw1999, as = "text", encoding = "UTF-8") 
unpacked_2018 <- httr::content(raw2018, as = "text", encoding = "UTF-8") 
...
```{r}
rawROMO <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=1&startYear=2010&endMonth=12&endYear=2021")
unpacked_ROMO <- httr::content(rawROMO, as = "text", encoding = "UTF-8") 
final_ROMO <- jsonlite::fromJSON(unpacked_ROMO)
...
```{r, eval = F}
parkwide_vistation <- function(year = 2021){
...
test <- parkwide_vistation(year = 2012)
```
...
```{r}
unit_visitation <- function(startyear, endyear, parkcode){
  raw <- httr::GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=",parkcode,"&startMonth=1&startYear=",startyear,"&endMonth=12&endYear=",endyear))
...
test2 <- unit_visitation(startyear = 1992, endyear = 2009, parkcode = "ROMO")  
...
```{r}
romo <- unit_visitation(startyear = 1990, endyear = 2021, parkcode = "ROMO")
ever <- unit_visitation(startyear = 1990, endyear = 2021, parkcode = "EVER")
```
...
```{r}
#I left the months at 1 and 12 in the URL in my original unit_visitation(). I now realize that I cannot change them by doing it this way. I could have put them in as defaults in this step so that it would be possible to change them in the future. 
```
...
output_parks <- vector("list", length = length(parks))
for( i in 1:length(parks)){
  
   output_parks[[i]] <-
    unit_visitation(parkcode = parks[i], startyear = 1980, endyear = 2021)
  
}

allparks <- dplyr::bind_rows(output_parks)
...
output_map <-  parks %>% 
  map(~ unit_visitation(parkcode = ., startyear = 1980, endyear = 2021))

identical(output_parks,output_map)

allparks <- dplyr::bind_rows(output_map)
```

<!--chapter:end:Week-3-1.Rmd-->
...
```{r}
unit_visitation <- function(startyear = 1980, endyear = 2021, parkcode){
  raw <- httr::GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=",parkcode,"&startMonth=1&startYear=",startyear,"&endMonth=12&endYear=",endyear))
...
... except, the rows in `parkwide`'s UnitCode and UnitName columns are empty. 😑 Let's fix the `UnitCode` column to list "Parkwide" using `mutate()` and an `ifelse()` statement:
...
```{r}
yearlyungrouped <- yearly %>% 
  ungroup() %>% 
  mutate(total = ifelse(yearly$UnitCode == "Parkwide","Parkwide","individ"))

plotly::ggplotly(
  ggplot(data=yearlyungrouped) +
    geom_point(aes(x = Year, y = RecVisitation, color = UnitCode)) +
    geom_path(aes(x = Year, y = RecVisitation, color = UnitCode)) +
    scale_y_continuous(labels = scales::label_scientific()) +
    facet_wrap(vars(total), scales = "free_y") +
    theme_bw(base_size=10)
  )
```
...
wide_data_percent <- wide_data %>% 
  mutate_at(.vars = vars(2:6, 8:10), ~. /Parkwide*100)

narrow_percent <- wide_data_percent %>% 
  select(., Year:OLYM,ROMO:ZION) %>% 
  pivot_longer(cols = -Year,
               names_to = "Park",
               values_to = "RecVisitationPercent")

plotly::ggplotly(
  ggplot(data=narrow_percent) +
    geom_point(aes(x = Year, y = RecVisitationPercent, color = Park)) +
    geom_path(aes(x = Year, y = RecVisitationPercent, color = Park)) +
    scale_y_continuous() +
    theme_bw(base_size=10)
  )
```
...
joined_data_year <- joined_data %>% 
  ...
  summarise(parkvis = sum(RecreationVisitors.x + NonRecreationVisitors.x),
           totalvis = sum(RecreationVisitors.y + NonRecreationVisitors.y)) %>% 
  mutate(., ParkPercent = (parkvis/totalvis)*100) 

plotly::ggplotly(
  ggplot(data=joined_data_year) +
    geom_point(aes(x = Year, y = ParkPercent, color = UnitCode.x)) +
    geom_path(aes(x = Year, y = ParkPercent, color = UnitCode.x)) +
    scale_y_continuous() +
    theme_bw(base_size=10)
  )
...
```{r}
finalwide <- joined_data_year %>% 
  pivot_wider(., names_from = UnitCode.x, values_from = ParkPercent) %>% 
  select(., !(2:3)) %>% 
  filter(Year >= 1990) %>% 
  colMeans(., na.rm = TRUE)
...
occ <- st_as_sf(sp_df, coords = c('decimalLongitude','decimalLatitude'), crs = 4326)
```
...
```{r}
larcoSNOTEL <- SCAN_SNOTEL_metadata %>% 
  filter(., County == "Larimer", Network == "SNOTEL")

snotel_sites <- st_as_sf(larcoSNOTEL, coords = c('Longitude','Latitude'), crs = 4326)

COSNOTEL <- SCAN_SNOTEL_metadata %>% 
  filter(., State == "Colorado", Network == "SNOTEL")
```

How many SNOTEL sites are located in Colorado?

There are 115 SNOTEL stations in Colorado. Only 8 of those are in Larimer County, CO. 
...
allsnosites <- purrr::map(snotel_sites$Site,sno_func) %>% 
  bind_rows()
...
```{r}
save(counties, rivers, occ, allsnosites, file = "data/spatdat.RData")
```
...
elev_crp <- crop(Elevation, ext(counties))
writeRaster(elev_crp, "data/elevation.tif", overwrite = TRUE)
...
The auxiliary file is the metadata for the land cover file. It is sort of like a key. The land cover data is different than the elevation because the elevation data is a DEM and the land cover data gives information about what is on the ground. The colors are also different and the land cover data requires a legend/key. 

<!--chapter:end:Week-4-1.Rmd-->
...
```{r}
library(mapview)
#AP_stationlist <- read_csv("~/WR440/AP_stationlist.csv") %>%
  #rename(., Longitude = dec_long_va, Latitude = dec_lat_va, Name = station_nm, Site = site_no) %>%
...
#sno_sf <- st_as_sf(COSNOTEL, coords = c("Longitude","Latitude"), crs = 4326)

#usgs_sf <- st_as_sf(AP_stationlist, coords = c("Longitude","Latitude"), crs = 4326)

#hopefully these two lines allow you to read in the shp files I created with this ^^^ code. Those are the two I used to make the map. I also saved a pic of the map ("testsnomap.png")
st_read("Week-4-Geospatial-helenflynnn/usgs.shp")
st_read("Week-4-Geospatial-helenflynnn/sno.shp")

testsnowmap <- tm_shape(counties)+
  tm_polygons(col = "STATEFP",
              palette = "white")+
tm_shape(sno_sf)+
  tm_dots(shape = "Network",
          alpha = 0.75,
...
tmap_save(testsnowmap,"testsnowmap.png")
  
mapview::mapview(sno_sf, zcol = "Name")+
  mapview(usgs_sf, zcol = "Name", color = 'magenta')

#st_write(usgs_sf, "C:/Users/helen/Documents/ESS523A/Week-4-Geospatial-helenflynnn/usgs.shp")
#st_write(sno_sf, "C:/Users/helen/Documents/ESS523A/Week-4-Geospatial-helenflynnn/sno.shp")

```


<!--chapter:end:Week-4-3.Rmd-->
